...
CERT_DIR_ON_HOST="docker/certificates"
...
docker exec -it -e KEYSTORE_PASSWORD="${KEYSTORE_PASSWORD}" \
    $CONTAINER_NAME /bin/bash -c "ls -l /scripts; ls -l /certs; sudo /scripts/build.sh ${KEYSTORE_PASSWORD} ${ELASTIC_PASSWORD} ${ELASTIC_INTERNAL} ${ELASTICSEARCH_HOST}"
